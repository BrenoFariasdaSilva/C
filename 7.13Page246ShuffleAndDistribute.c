...
7.13 Projeto: embaralhar e distribuir cartas. Use
as funções desenvolvidas no Exercício 7.12 para escrever
um programa que trate duas mãos de pôquer
com cinco cartas; avalie cada mão e determine qual
é a melhor.
...
void Shuffle (int Deck[Line][Column][Players]) // Shuffle (Baralha) o Deck.
{
    ...
    for (Player = 0; Player < Players; Player++) //Deck do Player.
    {
...
void Flush (const int Deck[Line][Column][Players], const char *Suit[], int PlayerPoints[]) //Procura se há um Flush (cartas com Suit(Naipe) consecultivos).
{
